...
Foi utilizada a biblioteca customizada `pwm_z42` (acesso direto aos registradores do TPM2), em vez do *driver* PWM do Zephyr via Device Tree. Essa escolha foi deliberada: o suporte do Zephyr ao PWM via DTS para o KL25Z não está disponível nesta versão do framework (a própria documentação de apoio da disciplina recomenda a abordagem por registradores para o módulo TPM), e a tentativa de configurar `TPM2_CH0`/`TPM2_CH1` via *overlay* DTS exigiria suporte de driver que não existe para esse SoC no PlatformIO/Zephyr atual.

A biblioteca `pwm_z42` encapsula o acesso a SIM, PORT e TPM em funções (`pwm_tpm_Init`, `pwm_tpm_Ch_Init`, `pwm_tpm_CnV`), de modo que o `main.c` não manipula endereços de registrador diretamente — apenas chama essas funções, mantendo a lógica de aplicação (cor laranja, leitura do terminal, cálculo de duty cycle) desacoplada do acesso a hardware.
...
```text
https://github.com/Beserrovsky/PSI3441-ATV2_pwm-zephyr
```
